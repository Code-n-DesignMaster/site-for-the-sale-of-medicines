...
const billingFormValidator = values => {
  ...
  // console.log(String(values.cardNumber).replace(/\s/g,''));
  // console.log(creditCartType(String(values.cardNumber).replace(/\s/g,'')));
  // console.log(!creditCartType(String(values.cardNumber).replace(/\s/g,'')).filter(type => allowedCardTypes.includes(type.type)).length > 0);
  // console.log(allowedCardTypes);
  if (!creditCartType(String(values.cardNumber).replace(/\s/g, '')).filter(type => allowedCardTypes.includes(type.type)).length > 0) {
    errors.cardNumber = 'Card type is invalid';
  }
